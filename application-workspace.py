...
@login_required
def history():
    transactions = db.execute("SELECT * FROM history WHERE userid = :id ORDER BY time", id = session["user_id"])
...
def rankings():
    ...
    for player in players:
        ...
        player["total"] = usd(sum + player["cash"])
        player["cash"] = usd(player["cash"])
    players = sorted(players, key = lambda k: k["cash"], reverse=True)[:size]
    for r in range(size):
...
def sell():
    if request.method == "GET":
        ...
        if not result:
            return apology("Invalid Symbol")
        purchase = db.execute("SELECT * FROM purchases WHERE symbol = :symbol", symbol = result["symbol"])
